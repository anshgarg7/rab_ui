...
class login extends Component {
    ...
    login_api = (event) => {
        ...
        http.Login_APi(`v1/hotel/login`, urlencoded)
            .then(result => {
                let { data } = result
                if (data && data.status === 200) {
                    let user = { token: data.data.token, name: data.data.first_name, img: data.data.image }
                    auth.login(user)
                    this.setState({loder:false})
                        ...
                        <Container fluid>
                            ...
                            <Form onSubmit={this.login_api} className="label_form mt-4"  >
                                <Row>
                                    ...
                                    <Col lg={2}>
                                        <Form.Group className="mt-3 mb-3 ">
                                            <Link to="/forgotPassword"><Form.Label style={{ marginLeft: "3vh", cursor: "pointer" }} className="text-white ml-4">Forget Password ?</Form.Label></Link>
                                        </Form.Group>
                                    </Col>
                                    <Col lg={3}></Col>
                                </Row>
